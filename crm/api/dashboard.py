...
@frappe.whitelist()
def get_voice_stats():
	"""Return today's key metrics for Voice AI CRM dashboard cards."""
	...
	calls_today = frappe.db.count(
		"CRM Call Log",
		{"creation": ["between", [today + " 00:00:00", tomorrow + " 00:00:00"]]},
	)

	open_statuses = frappe.get_all("CRM Lead Status", {"type": "Open"}, pluck="name")
	new_leads_week = frappe.db.count(
		"CRM Lead",
		{"creation": [">=", week_start + " 00:00:00"], "status": ["in", open_statuses]} if open_statuses
		else {"creation": [">=", week_start + " 00:00:00"]},
	)

	appointments_today = frappe.db.count(
		"Voice Appointment",
		{
			"scheduled_at": ["between", [today + " 00:00:00", tomorrow + " 00:00:00"]],
			"status": ["!=", "cancelled"],
		},
	) if frappe.db.exists("DocType", "Voice Appointment") else 0

	pending_support = frappe.db.count(
		"CRM Call Log",
		{"status": "Missed", "type": "Incoming"},
	)
