<template>
  <LayoutHeader>
    <template #left-header>
      <ViewBreadcrumbs v-model="viewControls" routeName="All Customers" />
    </template>
  ...
  >
    <template #title="{ titleField, itemName }">
      <div class="flex items-center gap-2">
        ...
        <div
          v-if="['modified', 'creation', 'first_response_time', 'first_responded_on', 'response_by'].includes(titleField)"
    ...
    <template #fields="{ fieldName, itemName }">
      <div v-if="getRow(itemName, fieldName).label" class="truncate flex items-center gap-2">
        ...
        <div
          v-if="['modified', 'creation', 'first_response_time', 'first_responded_on', 'response_by'].includes(fieldName)"
          class="truncate text-base"
        >
...
const { getFormattedPercent, getFormattedFloat, getFormattedCurrency } = getMeta('CRM Lead')
...
const rows = computed(() => {
  ...
  if (leads.value.data.view_type === 'group_by') {
    ...
    return getGroupedByRows(leads.value?.data.data, leads.value?.data.group_by_field, leads.value.data.columns)
...
function getGroupedByRows(listRows, groupByField, columns) {
  ...
  groupByField.options?.forEach((option) => {
    ...
    if (groupByField.fieldname == 'status') {
      groupDetail.icon = () => h(IndicatorIcon, { class: getLeadStatus(option)?.color })
    }
...
function parseRows(rows, columns = []) {
  ...
  return rows.map((lead) => {
    ...
    leads.value?.data.rows.forEach((row) => {
      ...
      if (row == 'lead_name') {
        _rows[row] = { label: lead.lead_name, image: lead.image, image_label: lead.first_name }
      } else if (row == 'organization') {
        _rows[row] = lead.organization
      } else if (row === 'website') {
        _rows[row] = website(lead.website)
      } else if (row == 'status') {
        _rows[row] = { label: lead.status, color: getLeadStatus(lead.status)?.color }
      } else if (row == 'sla_status') {
        let value = lead.sla_status
        let tooltipText = value
        let color = lead.sla_status == 'Failed' ? 'red' : lead.sla_status == 'Fulfilled' ? 'green' : 'orange'
        ...
        _rows[row] = { label: lead.lead_owner && getUser(lead.lead_owner).full_name, ...(lead.lead_owner && getUser(lead.lead_owner)) }
      } else if (row == '_assign') {
        let assignees = JSON.parse(lead._assign || '[]')
        _rows[row] = assignees.map((user) => ({ name: user, image: getUser(user).user_image, label: getUser(user).full_name }))
      } else if (['modified', 'creation'].includes(row)) {
        _rows[row] = { label: formatDate(lead[row]), timeAgo: __(timeAgo(lead[row])) }
      } else if (['first_response_time', 'first_responded_on', 'response_by'].includes(row)) {
        let field = row == 'response_by' ? 'response_by' : 'first_responded_on'
        _rows[row] = {
          label: lead[field] ? formatDate(lead[field]) : '',
          timeAgo: lead[row] ? (row == 'first_response_time' ? formatTime(lead[row]) : __(timeAgo(lead[row]))) : '',
        }
      }
    })
...
function showNote(name) {
  showModal({
    ...
    callbacks: { afterInsert: () => {}, afterUpdate: () => {} },
  })
}
