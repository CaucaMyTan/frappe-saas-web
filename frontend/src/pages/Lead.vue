...
function triggerAICall() {
  toast.success(__('Đang kết nối tới n8n để kích hoạt cuộc gọi...'))
  call('voice_crm.voice_api.trigger_outbound_call', {
    lead_id: props.leadId
  }).then((res) => {
    toast.success(__('Cuộc gọi đã được kích hoạt thành công!'))
  }).catch((err) => {
    console.error(err)
    // Error is handled by frappe call internally mostly, but just in case
  })
}
